...
Successful Jenkins setup evidence uses private `registry.cloudbase.it`, the `appliance` namespace, `kolla_branch: 2023.1-eol`, `kolla_openstack_release: 2023.1`, and Kolla tag `2023.1`. A separate runtime snapshot of one `2603.4` appliance of unknown origin observed the complete private Kolla image names `barbican-api`, `barbican-keystone-listener`, `barbican-worker`, `keystone`, `keystone-fernet`, `keystone-ssh`, `rabbitmq`, `memcached`, `mariadb-server`, and `kolla-toolbox`, all tagged `2023.1-ubuntu-jammy`. It also observed `smallstep/step-ca:latest` and `influxdb:1.7`. This is not proof of RC2 content, registry availability, image digests, or source provenance; see [Appliance Runtime](appliance-runtime.md).
