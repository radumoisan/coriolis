...
The following status entries are chronological. Later entries supersede earlier local or deployed-state claims, including the old `0.5.3` history, unless they explicitly describe a retained historical fact.
...
- MariaDB runtime evidence is committed locally at `86287ad` on `dev`, unpushed and undeployed. Inspection and disposable local validation of the exact approved digest proved restricted direct non-Galera startup, value-safe idempotent bootstrap, authenticated checks, and durable-volume recreation; the Kolla startup and Galera checks were rejected. This is not a Kubernetes workload contract: storage/PVC ownership and sizing, generated manifests, probes, resources, lifecycle/recovery, and reliable log capture remain fail-closed blockers. Validation passed with 252 tests plus Ruff, mypy, Helm, and diff checks; no runtime, CRD, RBAC, release, deployment, or cluster behavior changed.
- The documentation-only MariaDB Kubernetes development contract is committed locally at `369cc97` on `dev`, unpushed and undeployed. It freezes future CR-configurable storage class, size, CPU, and memory; a retained ownerless RWO Filesystem PVC; one owner-referenced StatefulSet; file-only bootstrap and probes; direct console logging; and mutation-free reconciliation ordering. The next slice is pure schema/resolver and builder/preflight implementation. Target-storage validation and production backup/restore, HA, and RPO/RTO remain open; no CRD, runtime, RBAC, workload, release, deployment, or cluster behavior changed.
- The pure MariaDB desired-state preparation is committed locally at `5a2dfce` on `dev`, unpushed and undeployed. It adds the optional storage/resources CRD shape, quantity-safe resolver, value-safe direct-MariaDB configuration/bootstrap renderers, exact retained-PVC reuse classification, deterministic PVC/ConfigMap/Secret/StatefulSet manifests, and metadata-first collision preflight. The generated resources are not wired into `main.py`; reconciliation, PVC/StatefulSet RBAC, target-storage validation, and production acceptance remain open. Validation passed with 286 tests plus Ruff, mypy, Helm, and diff checks; deployed `0.5.3` remains marker-only.
- The development stack through MariaDB reconciliation is now published on `origin/dev`: `55212b0` implements reconciliation and `6ba9c7e` records publication, superseding the pre-publication status in the historical entries above. Complete settings resolve before Kubernetes client construction; reconciliation preserves ordered reads and preparation-before-write, creates an absent PVC or performs exact no-write reuse, guarded-applies the managed ConfigMap, Secret, and StatefulSet, and writes the marker last. PVC RBAC is `get`/`create`; StatefulSet RBAC is `get`/`create`/`patch`. Validation passed before publication with 316 tests plus Ruff, mypy, Helm, and diff checks. CIXpress commit `cacab93` synchronized chart version, app version, and image tag to `0.5.4`; this is not artifact, pipeline-success, deployment, or runtime-readiness evidence. Target-storage validation and production backup/restore, HA, and RPO/RTO remain open; deployed `0.5.3` remains marker-only and `Ready=False/RuntimeNotImplemented` remains truthful.
- Operator `0.5.5` clean first boot exposed anonymous MariaDB initialization accounts shadowing `coriolis@%`. Commit `3ee5d2d` adds `--skip-test-db`; CIXpress pipeline `kpv306` completed all four expected steps and released tested artifact `0.5.6` at CI-owned commit `647c63b`. Released `0.5.6` passed a clean isolated single-node `local-path` POC without repair: PVC Bound in 4 seconds, Pod Ready in 17 seconds, zero anonymous accounts/test database, `fsGroup: 42434` writes, authenticated probes, exact no-write PVC and retained-credential reuse across CR recreation, persisted same-node remount, and normal 12-second pod deletion. Final evidence commit `2526d33` was published successfully by pipeline `3s6wkx`; CI synchronized docs-only version `0.5.7` at `d09bde1`. At the final observation Argo still ran tested `0.5.6` alongside legacy `0.5.3` in `coriolis`; their selectors overlapped, blocking shared-namespace appliance testing, and no appliance CR existed. All POC resources and the PV were cleaned up. CSI/cross-node evidence and production backup/restore, HA, and RPO/RTO remain open; `Ready=False/RuntimeNotImplemented` remains truthful.
- Memcached reconciliation source `063e438` was published by pipeline `4dcpfk`, with all four expected steps successful, as POC-tested release `0.5.8` at CI-owned commit `cb6b055`. The isolated single-node POC validated the exact restricted one-replica ephemeral Deployment, Ready Service endpoint, Service-DNS protocol set/get, and a normal Pod replacement Ready in 3.603 seconds with a distinct UID, zero restarts, the same approved image digest, and loss of the original cache key. CR-to-Deployment took 44 seconds and Deployment-to-Ready took 3 seconds; MariaDB and Memcached reached `1/1`, while `Ready=False/RuntimeNotImplemented` remained truthful. Normal cleanup removed the CR, Helm release, namespace, retained PVC, copied registry Secret, and Delete-policy PV; the node remained Ready/schedulable and zero appliance CRs remain. Argo ran only healthy `coriolis-operator:0.5.8` at the POC cleanup observation, with no legacy selector overlap. Final evidence commit `edad291` generated docs-only `0.5.9` at `9c6dc88`; pipeline `ib7uf5` reached top-level `SUCCEEDED` and one terminal snapshot showed all expected steps `SUCCEEDED`, but intermediate and later detail responses were inconsistent or omitted cleaned-up steps, so per-step monitoring confirmation remains qualified. The final external check showed Argo running healthy docs-only `0.5.9`; the POC-tested artifact remains `0.5.8`. RabbitMQ evidence followed next.
- RabbitMQ reconciliation source `6a5a2b5` was published by successful pipeline `qhvqt1` as release `0.5.10` at CI-owned commit `c48fd79`, but its bootstrap/authenticated-AMQP POC was not accepted because readiness at period `5s`/timeout `5s` produced 37 timeouts and Endpoint flapping. Fix `0d52c57` changed readiness to `10s`/`15s`; successful pipeline `4nj6f5` released POC-tested `0.5.11` at `3985a67`. Its accepted isolated single-node `local-path` POC validated the retained RWO Filesystem PVC, fsGroup writes, restricted StatefulSet, Service/EndpointSlice, authenticated durable AMQP, stable readiness, normal Pod replacement and same-node persistence, exact no-write retained reuse across CR recreation, and clean-storage bootstrap. Final cleanup removed the disposable release, namespace, helpers, and PVs; the node remained Ready/schedulable, zero appliance CRs remained, and `Ready=False/RuntimeNotImplemented` stayed truthful. Final evidence commit `0866deb` was published by pipeline `4bp7ht`, with all four expected steps confirmed `SUCCEEDED`; CIXpress synchronized docs-only version `0.5.12` at `355654f`. CSI/cross-node attach-detach, backup/restore, HA, RPO/RTO, credential rotation, and production storage remain open. Keystone runtime evidence followed and is recorded next.
- Keystone reconciliation source `f90cae4` was published by successful pipeline `c2pajn`, with all four expected steps successful, as release `0.5.13` at CI-owned commit `7eb2215`. That `0.5.13` POC was not accepted because non-root UID/fsGroup `42425` could not chmod root-owned emptyDir mount roots before db-sync/bootstrap; dependency containers stayed healthy. Fix `087bbc2` passed 398 tests plus Ruff/mypy/diff and a successful all-step pipeline `fhzlg6`, releasing POC-tested `0.5.14` at CI-owned commit `edd349a`. A local direct Helm OCI pull returned `401` before any disposable Helm release/CR existed, so a disposable Argo Application promoted exact `0.5.14` using the existing private-registry integration and `skipCrds`, with no credential exposure. The accepted isolated single-node `local-path` POC validated approximately 90 seconds CR-to-Keystone Available, init exit `0`, `/v3` returning `200`, authenticated `201` with token header presence, restricted one-replica Recreate strategy, Service/EndpointSlice, zero restarts, Pod replacement with the same approved digest, retained Secrets with exact metadata, and CR recreation reusing the exact retained Secrets/PVCs without writes and reaching Available in roughly 37 seconds, with collision mutation-free and recovery behavior. Cleanup removed the CR, Application, namespace, retained state, and Delete-policy PVs; the node stayed Ready/schedulable with no POC workload or CR remaining. Final evidence commit `0ccb22a2596bc48a41adbfb42e560f12409da0d0` was published by pipeline `h7fntw` with all four expected steps `SUCCEEDED`; CIXpress synchronized docs-only release `0.5.15` at `f7c369dc9ef52de97e3a1cbef32aebac14bdde76`. At the final `0.5.15` observation, the shared Argo deployment was Synced/Healthy with the operator `1/1` Ready and zero restarts, and no appliance CR existed. `Ready=False/RuntimeNotImplemented` remained truthful. CSI/cross-node, backup/restore, HA, RPO/RTO, credential/key rotation, and production storage remained open; Coriolis-common bootstrap followed.
- Coriolis-common initial source `391f94c` was published by pipeline `v2nv5t` as release `0.5.16` at `5438193`; its v1 POC failed provider-continuation parsing before dbsync, with the correct `BootstrapFailed`, no marker, and no mutation. Fix source `764b9952d1e1c9bc1cbce08afddea8781f391f42` was published by pipeline `ectoq4`, with all four expected steps successful, as release `0.5.17` at `6bfc494d50949b7a5aa770c4febb7c5100b3b363`. The accepted v2 exact conductor Job completed in 31 seconds with exit `0` and zero restarts; dependencies were ready; independent schema/Keystone verification passed twice; and v2 was final, with only `Ready=False/RuntimeNotImplemented`. Operator resume exactly reused the completed Job and ConfigMap without writes; five Secrets and two PVCs were retained exactly across CR recreation; and collision caused zero writes. Initial automatic-recovery source `c89b57fcd5b2ee160c1a0986be9fedc9edc19ec8` was published by successful pipeline `mg04ql` as release `0.5.19` at `3a2af04270762069f3d50905ad6f4255d699c302`, but its POC was not accepted: the 60-second timer began recovery in 28 seconds, then dropped its framework retry after status changed to `BootstrapRunning`, leaving a succeeded Job's CR unreconciled for over 10 minutes. Corrective source `5f52c7004b3b393d85c10b07e9301d3fa3587164` was published by successful pipeline `0km4kz` as accepted release `0.5.20` at `61c7b3a0d0cea2e81b290023fa8ee5605e2ff261`. Its fresh isolated POC held the unmanaged immutable collision for 135 seconds over two timer intervals with unchanged conflict and CR resourceVersions, zero appliance children, and no operator restart; removing only the conflict began recovery in 51.763 seconds and reached final reconciliation in 139.982 seconds across `BootstrapRunning`, without a user CR/spec change or generation change. Final status was `acceptedVersion=2603.4`, `Accepted=True`, `Reconciled=True`, `Degraded=False`, and `Ready=False/RuntimeNotImplemented`; the Job succeeded with exit `0` and zero restarts, all four dependencies were `1/1`, and each Service had one ready EndpointSlice. Normal cleanup removed the exact Application, namespace, CR, retained resources, and both Delete-policy PVs without force. Evidence commit `4cf9f1674b112010b17f7244001c29fa1022bd42` was published by pipeline `wecmnz`, with all expected steps confirmed successful, as docs-only release `0.5.21` at `e06bc95492b04a453f09cd12698e78638a22b825`. At the final observation, shared Argo target `0.*.*` resolved to tested `0.5.20`, `Synced/Healthy`, with the operator ready/available `1/1`; no POC Application, namespace, CR, or PVs remained; and the node was Ready/schedulable. This remains single-node `local-path` evidence only, and production gates remain open.
- Coriolis API reconciliation source `5a06759d6e0332345262f990b2fb56d9d82fbef7` was published by CIXpress pipeline `44muez`, with all four expected steps successful, as POC-tested release `0.5.22` at CI-owned commit `ef86249cecee812482a96da8f41b1985d7cb764b`. The accepted isolated POC in `coriolis-api-validation-20260823` validated the owner-referenced ClusterIP Service and ready EndpointSlice on TCP `7667`, the restricted one-replica `Recreate` Deployment at exact API digest `sha256:fce6369f07ef777b5174d3a4f849d4eac914256a20a47ffa0cd1c98081be2705`, direct non-root startup, zero restarts, and Service-DNS `/v1` returning the expected unauthenticated `401`. Unmanaged Service and Deployment collisions each remained mutation-free and without status churn for more than one timer interval, then recovered automatically after conflict removal without a CR edit or operator restart. CR recreation reused five retained Secrets and both PVCs exactly without writes; a normal API Pod replacement was Ready in about 5 seconds with the same imageID and `401`. Cleanup removed the CR, exact Application, namespace, and both Delete-policy PVs normally, with no POC resources left. Final evidence commit `e7db48dafe4885645e87c7ceb07909609e3a013a` was published by pipeline `ejmk4n`, with all four expected steps confirmed `SUCCEEDED`; CIXpress synchronized documentation-only release `0.5.23` at `7938827be432462853e0f829cb905319ac1b2b7b`. Shared Argo now resolves `0.5.23`, is `Synced/Healthy`, and runs operator `1/1` with no appliance CR.
- Coriolis conductor reconciliation source `d7abd778937203c97eb7501c2a2dc1a0c05cd5c4` was published by CIXpress pipeline `nd4xu7`, with all four expected steps successful, as POC-tested release `0.5.24` at CI-owned commit `56c0b29dbfc2883633b9cd20578ecae7f124b5a7`. The accepted isolated POC in `coriolis-conductor-validation-20260823` held an unmanaged conductor Deployment collision mutation-free for 130 seconds over two timer intervals, then recovered without a CR edit or operator restart about 148 seconds after conflict deletion. The managed one-replica `Recreate` Deployment ran the direct restricted conductor at exact digest `sha256:27495f44fbb8b320098d0aa04cd9dcb2a4b432e57aa17417606efc5403ac09c7`, with no Service or probes. Output-free authenticated verification proved API-to-RabbitMQ-to-conductor-to-MariaDB `GET /v1/{project_id}/endpoints` HTTP `200` before and after a normal conductor Pod replacement in about 6 seconds; unauthenticated API remained `401`. CR recreation reused five retained Secrets and both Bound PVCs exactly without writes and reconverged in about 71 seconds. Normal cleanup removed the CR, Application, namespace, and both Delete-policy PVs without force, finalizer changes, or individual retained/PVC/PV deletion.
- Final conductor evidence commit `770f0448db61fac3de34740420cdefcd7428be19` was published by pipeline `utsrw6`, with all four expected steps confirmed `SUCCEEDED`; CIXpress synchronized documentation-only release `0.5.25` at `8bafc830a20ba923b1841d8e3a3cb61d9f11b60b`. Shared Argo resolves `0.5.25`, is `Synced/Healthy`, and runs operator `1/1` with zero restarts and no appliance CR. Authenticated read-only conductor RPC is accepted; write workflows and scheduler/worker/transfer-cron/minion/deployer workloads remain absent. `Ready=False/RuntimeNotImplemented` remains truthful. The remaining application workloads, then Ingress, are next; production CSI/cross-node, backup/restore, HA, RPO/RTO, and rotation gates remain open.
