...
- The documentation-only multi-resource reconciliation policy is committed locally at `747427a` on `dev`, not pushed or deployed: validate profile/version, then pre-read the marker plus five resources and complete preflight/preparation before writes; `404` is `ABSENT`; use canonical create/guarded SSA, retain `REUSE` without writes, and retry `AlreadyExists`/`Conflict` from fresh preflight. There is no rollback, compensation, or deletion: stop at the first failure, preserve earlier writes, set value-safe `ResourceReadFailed`/`ResourceApplyFailed`/`MarkerApplyFailed` status, then let the framework retry; stable `ResourceCollision` is no-mutation/non-transient. The marker is last and only records foundational completion while `Ready=False`/`RuntimeNotImplemented`; no `main.py`, runtime, Kubernetes, RBAC, SSA, status-retry, CRD, release, or deployment behavior changed, and deployed `0.5.3` remains marker-only.
- No core workloads are implemented or deployed; the immediate next implementation is collision-safe runtime reads and create/guarded SSA for all five resources plus marker, minimal Secret/ConfigMap get/create/patch RBAC, status-then-Kopf-retry wiring, and exhaustive tests. Runtime remains blocked; retained adoption/runtime construction and deferred TLS/provider/dependency/bootstrap/storage/readiness/rotation remain deferred.
