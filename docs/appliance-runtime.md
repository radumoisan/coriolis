...
- No core workloads are implemented or deployed; next is collision-safe preflight for all five frozen resources before mutation, metadata classification before retained Secret semantic validation, semantic failures fail-closed to `COLLISION`, generators only for `ABSENT`, then SSA/minimal Secret RBAC/reconciliation-status semantics; TLS/optional credentials/storage/probes/readiness/bootstrap/rotation are deferred.
