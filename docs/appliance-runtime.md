...
- No core workloads are implemented or deployed; the immediate next implementation is collision-safe runtime reads and create/guarded SSA for the marker plus two retained credential Secrets and the owned configuration ConfigMap/Secret, minimal Secret/ConfigMap get/create/patch RBAC, status-then-Kopf-retry wiring, and exhaustive tests. Runtime remains blocked; Services, Ingress, workloads, retained adoption/runtime construction, and deferred provider/bootstrap/storage/readiness/rotation remain later work.
