...
- No core workloads are implemented or deployed; the immediate next gate is frozen multi-resource read/apply failure handling, reconciliation status, atomicity, and marker-last semantics; only then come runtime collision-safe pre-reads for all five resources, minimal Secret/ConfigMap RBAC, and SSA. Runtime integration is not unblocked; retained adoption/runtime construction and deferred TLS/provider/dependency/bootstrap/storage/readiness/rotation remain deferred.
