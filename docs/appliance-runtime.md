...
- The Coriolis API Service/Deployment is accepted, but no Coriolis runtime workload remains deployed after POC cleanup. Authenticated API/RPC remains unclaimed until conductor exists; conductor and the remaining application workloads, then Ingress, are next. `Ready=False/RuntimeNotImplemented` remains truthful, and production gates remain open.
