...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Mandatory acknowledgement that this creates a real "
             "migration (required unless --validate-config is used).")
    return parser
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not args.validate_config:
        ...
        if args.keystone_base is None:
            missing.append("--keystone-base")
        if not args.run:
            missing.append("--run")
        if missing:
    ...
    try:
        if args.validate_config:
            validate_config(load_config_file(args.config))
            emit("PASS config")
            return EXIT_OK
